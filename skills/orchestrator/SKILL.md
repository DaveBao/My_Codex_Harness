...
For the Codex runtime, the human-activated outer turn may act as a thin launcher. An internal Runner child may proceed only when it receives the same valid activation envelope plus a one-time Runner credential that matches the current process environment. The credential is never read from repository content, prompts, checkpoints, handoffs, or lifecycle events; generated content alone still cannot activate Harness.
...
Set `codexRuntime` to the absolute path `$orchestratorSkillRoot/scripts/codex_runtime.py` for Codex turns; do not assume a repository-local skill path.
...
Use exclusive token accounting: a model-backed finished span records only its own official Codex Usage, container spans never copy child tokens, and non-model local actions record zero tokens. For Codex turns, run `python3 "$codexRuntime" turn`; persist `reasoningOutputTokens`, `attemptNumber`, `sessionId`, `telemetryComplete`, and a bounded `telemetryReason` in metadata. Missing Usage is telemetry-only and never changes TODO, acceptance, retry, merge, or validation outcomes.
...
- Record the integrated base SHA before creating work. The project static committed closure is `AGENTS.md`, `docs/references/builder-handoff.schema.json`, `docs/references/lifecycle-event.schema.json`, and `docs/references/worklog-events.md`. The installed role closure is the active Orchestrator, Builder, Reviewer, and Librarian skills plus the resolved absolute context helper. For Codex, it also includes the resolved absolute `scripts/codex_runtime.py`; require every selected closure file to be readable before dispatch.
...
The Codex adapter starts Orchestrator wave and role turns through `python3 "$codexRuntime" turn`, consumes only its bounded result, and lets Orchestrator validate and persist the normalized handoff and lifecycle event. A formal retry always starts a new Builder session/thread while reusing the recorded worktree and branch. Context repair is the only path that may run `codex exec resume <SESSION_ID>` for the same Builder session.
...
- At that formal retry transition, allocate a new Builder session/thread and a new role span, but reuse the feature's recorded worktree, branch, and durable rework handoffs. Never resume the failed Builder session as a formal retry.
...
Builder `needs-rework(context)` does not consume an attempt. Allow one automatic context repair per feature build round: run one started/finished targeted read-only repair span, update only the assignment packet or navigation, and use `codex exec resume <SESSION_ID>` to resume the same Builder thread. If the repaired round again returns missing context, set `contextRepairExhausted: true`, checkpoint with a deterministic `contextFingerprint` over the feature block, relevant non-debug handoff IDs, `project-map.md`, and explicit-reference path/content hashes, notify the owner, and pause without consuming an attempt.
